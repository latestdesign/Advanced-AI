...
class ModalityProjector(nn.Module):
    def __init__(self, cfg):
        ...
        self.inter_dim = self.lm_hidden_dim * 3                                                                    
        self.output_dim = self.lm_hidden_dim
        self.proj = nn.Linear(self.input_dim, self.inter_dim)
        self.act = nn.GELU()
        self.proj2 = nn.Linear(self.inter_dim, self.output_dim)
    ...
    def forward(self, x):
        ...
        """
        # Apply pixel shuffle, then 2 linear projection, separating by a Gelu activation.
